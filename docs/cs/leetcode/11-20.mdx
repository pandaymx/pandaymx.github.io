...
通过遍历字符串，遇见左括号时入栈，遇见右括号时出栈，最后栈为空则有效。
...
1. `stack = []` 初始化一个空栈，用于存储左括号。
2. `mapping = {")": "(", "}": "{", "]": "["}` 创建一个映射，用于快速查找右括号对应的左括号。
3. `for char in s`*：遍历字符串
    - `if char in mapping`：如果是右括号
    - `top_element = stack.pop() if stack else '#'`：获取栈顶元素
    - `if top_element != mapping[char]`：如果栈顶元素与当前右括号不匹配，返回 `false`
    - `else: stack.pop()`：如果匹配，则将栈顶元素弹出。
4. `return not stack`：如果栈为空，说明所有括号都已匹配，返回 `true`；否则返回 `false`。

**算法分析**

- **时间复杂度**: O(n)，其中 n 是字符串的长度。我们只需要遍历一次字符串。
- **空间复杂度**: O(n)，在最坏的情况下，如果字符串中全是左括号，我们需要将所有字符都压入栈中。
