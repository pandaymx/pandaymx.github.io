...
## Composite Actions（组合 Action）{#composite-actions}

Composite Action 允许将多个 step 封装成一个可复用的 action，适合逻辑复用场景。

### 创建 Composite Action {#create-composite-action}
...
### 使用 Composite Action {#use-composite-action}
...
## JavaScript Custom Action {#javascript-action}

JavaScript action 使用 Node.js 运行，适合需要复杂逻辑的场景。

### Action 项目结构 {#js-action-structure}
...
### action.yml 元数据 {#action-metadata}
...
### JavaScript Action 代码 {#js-action-code}
...
### 打包 JavaScript Action {#package-js-action}
...
## Docker Custom Action {#docker-action}

Docker action 在容器中运行，适合需要特定环境的场景。

### Docker Action 结构 {#docker-action-structure}
...
## Action 元数据语法 {#action-metadata-syntax}
...
## 复用工作流（Workflow Call）{#workflow-reuse}

通过 `workflow_call` 触发器，可以让一个工作流被另一个工作流调用，实现逻辑复用。

### 定义可复用工作流 {#define-reusable-workflow}
...
### 调用可复用工作流 {#call-reusable-workflow}
...
## Environment Files（环境文件）{#environment-files}
...
### 使用示例 {#environment-files-example}
...
## Concurrency Groups（并发控制）{#concurrency-groups}

控制同一时刻只允许一个工作流运行，避免资源冲突。

### 基础并发控制 {#basic-concurrency}
...
## Conditional Execution（条件执行）{#conditional-execution}
...
## Custom Outputs（自定义输出）{#custom-outputs}
...
## OIDC for Cloud Authentication（OIDC 云认证）{#oidc-authentication}
...
## GitHub API in Workflows（工作流中使用 GitHub API）{#github-api}
...
### 直接调用 GitHub API {#direct-api-call}
...
## 完整高级示例 {#complete-advanced-example}
