...
### ConfigMap YAML 定义 {#configmap-yaml}
...
## 🔄 使用配置：环境变量 vs 卷挂载 {#env-vs-volume}

### 方式一：环境变量 {#environment-variables}
...
## 🔐 外部密钥管理算子 {#external-secrets-operators}
...
#### 安装 External Secrets Operator {#install-eso}
...
## 🌍 多环境配置管理 {#multi-environment-config}

在生产实践中，通常需要管理多个环境（dev、staging、prod）的配置。

### 方案 1：命名空间隔离 {#namespace-isolation}
...
### 方案 2：Kustomize 覆盖 {#kustomize-overlay}
...
### 方案 3：Helm Values 文件 {#helm-values}
...
### 配置 Encryption Provider {#configure-encryption}
