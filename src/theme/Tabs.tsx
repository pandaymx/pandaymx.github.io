...
function TabsWrapper(props: React.ComponentProps<typeof Tabs>) {
  ...
  useEffect(() => {
    // 仅在 groupId 为 "os" 且没有初始 defaultValue 时执行自动检测
    if (groupId === 'os' && !defaultValue && ExecutionEnvironment.canUseDOM) {
      ...
      if (userAgentDataPlatform?.includes('mac') || ua.includes('mac')) {
        ...
        detected = 'linux';
      } else if (userAgentDataPlatform?.includes('win') || ua.includes('win')) {
        detected = 'windows';
      }
  ...
  const key =
    groupId === 'os' && selectedOs !== defaultValue ? selectedOs : undefined;
